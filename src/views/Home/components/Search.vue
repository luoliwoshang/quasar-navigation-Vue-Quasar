<template>
	<div>
		<div class="search-img" flex-center>
			...
			v-model="slide"
			@transition="(e)=>{changeSearchMethod(e)}"
			>
		...
		<div class="search-box">
			...
			<div class="search-content" v-show="showSearchContent" 	:style="{opacity}"	>
				<q-card
				square
	...
	export default {
		...
		data() {
			return {
				...
				search_baidu:"https://sp0.baidu.com/5a1Fazu8AA54nxGko9WTAnF6hhy/su?cb=sr",//百度的请求
				search_taobao:'https://suggest.taobao.com/sug?code=utf-8&_ksTS=1594451639078_2581&callback=jsonp2582&k=1&area=c2c&bucketid=1&q='//淘宝请求
			}
		},
		props:['opacity'],
		computed: {
		...
		methods: {
			...
			getDataSuccess(data) {
				...
				if(this.currentSearch==="baidu"){
					...
					let newdata=[]
					data.result.forEach((item,index)=>{newdata[index]=item[0]})
					this.arr=newdata
				}
			},
			itemClick(event) {
				if(this.currentSearch==="baidu"){
					window.open('https://www.baidu.com/s?wd=' + event.currentTarget.dataset.content)
				}else if(this.currentSearch==="taobao"){
					window.open('https://s.taobao.com/search?q='+event.currentTarget.dataset.content)
				}
			},
			submit() {
				if(this.currentSearch==="baidu"){
